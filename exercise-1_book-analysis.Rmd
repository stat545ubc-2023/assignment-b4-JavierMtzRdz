---
title: 'Exploring "The Divine Comedy": A Textual Analysis'
...
To begin with, we need to load the following packages for this assignment.
...
In this step, we download and clean the text of ["The Divine Comedy" by Dante Alighieri from the Project Gutenberg](https://dev.gutenberg.org/ebooks/8800).
...
In this phase, stop words are loaded and removed from the tokenized book. It is worth noting that two sources of stop words are used. The first source is the list from [tidytext](https://github.com/juliasilge/tidytext). However, Alighieri's book contains many old stop words not available in the tidytext's list. For that reason, the [Stephen Wittek's (MacGill) early modern stopwords](http://earlymodernconversions.com/computer-based-textual-analysis-and-early-modern-literature-notes-on-some-recent-research/) were also incorporated.
...
Here, the most common words in "The Divine Comedy" are extracted and visualized by part.

```{r fig.height=6, warning=FALSE, dpi=300}
